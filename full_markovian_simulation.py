...
def run_simulation(tree, root, queue, max_iterations):
    current_time = 0
    num_iterations = 0
    max_infected = 0
    ...
    while(not(queue.is_empty()) and num_iterations <= max_iterations):
        ...
        if len(tree.infected_nodes) > max_infected:
            max_infected = len(tree.infected_nodes)

    return current_time, max_infected
...
for i in range(num_simulations):
    ...
    finish_time, max_infected = run_simulation(tree,root,queue,max_iterations)
    
    finish_times.append(finish_time)
    root_hits.append(len(root.infection_times))
    max_infections.append(max_infected)
